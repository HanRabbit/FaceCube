...
class FaceDetectionApp(AIBase):
    def __init__(self, kmodel_path, model_input_size, anchors, confidence_threshold=0.5, nms_threshold=0.2,
    ...
    def send_data(self, box):
        with ScopedTiming("send_data", self.debug_mode > 0):
            if box:
                data = {"x": box[0], "y": box[1], "w": box[2], "h": box[3]}
            else:
                # 未检测到时均设置0
                data = {"x": 0, "y": 0, "w": 0, "h": 0}
            json_str = ujson.dumps(data, separators=(',', ':'))
            send_data(json_str + "\n")
...
if __name__ == "__main__":
    ...
    while True:
        with ScopedTiming("total", 1):
            ...
            box = face_det.draw_result(pl, res)  # 绘制结果
            face_det.send_data(box)
            pl.show_image()  # 显示结果
            gc.collect()  # 垃圾回收
